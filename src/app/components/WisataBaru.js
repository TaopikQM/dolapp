...
const WisataBaru = ({ router }) => {
  ...
  const filteredPlaces = places.filter(place => !ratings.hasOwnProperty(place.id) || ratings[place.id] === 0.0);
  ...
  return (
    <div className="relative w-full max-w-[1100px] mx-auto">
      {filteredPlaces.length > 0 && (
        <div>
          <h1 className="text-2xl font-bold text-center mt-4">Wisata Baru</h1>
                    ...
                    <div className="bg-white rounded-lg shadow-md" style={{ minHeight: '300px' }}>
                      <Carousel showThumbs={false} infiniteLoop useKeyboardArrows>
                        {place.imageUrls && place.imageUrls.map((imageUrl, index) => (
                          <div key={index}>
                            <img src={imageUrl} alt={`Image ${index + 1}`} className=" w-full object-contain rounded-lg" style={{ maxHeight: '300px' }} />
                          </div>
                        ))}
                        {place.videoUrls && place.videoUrls.map((videoUrl, index) => (
                          <div key={index}>
                            <video controls className=" w-full object-contain rounded-lg" style={{ maxHeight: '300px' }}>
                      ...
                      <Link href={`/detail/${encodeURIComponent(place.id)}`} key={place.id}>
                        <div className="p-4">
                          <p className="text-gray-600 text-sm">{place.category} | {place.wilayah}</p>
                          <h3 className="text-lg font-bold">{place.name}</h3>
                          <p className="text-gray-600 text-sm">{place.review}</p>
                          <div className="absolute top-0 bg-orange-500 text-white px-2 py-1 rounded-lg">
                              New {currentYear}
                          </div>
