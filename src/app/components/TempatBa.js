...
const TempatBa = ({ router }) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "tempat_wisata"), (snapshot) => {
      const fetchedPlaces = [];
      snapshot.forEach((doc) => {
        const placeData = doc.data();
        if (placeData.status === "ACTIVE") {
          fetchedPlaces.push({ id: doc.id, ...placeData });
        }
      });
  ...
  // Filter tempat berdasarkan nilai ReviewSum yang sama dengan 0.0
  ...
  return (
    <div className="relative w-full max-w-[1100px] mx-auto">
      {filteredPlaces.length > 0 && (
        <div>
          <h1 className="text-2xl font-bold text-center mt-4">Tempat Wisata Baru</h1>
                    ...
                    <div className="bg-white rounded-lg shadow-md" style={{ minHeight: '300px' }}>
                      ...
                      <Link href={`/detail/${encodeURIComponent(place.id)}`} key={place.id}>
                        ...
                        <div className="p-4">
                          ...
                          <h3 className="text-lg font-bold">{place.name}</h3>{/* Tambahkan label "Tahun Ini" di depan tengah atas */}
